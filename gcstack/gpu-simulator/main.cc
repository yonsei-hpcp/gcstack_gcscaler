...
int main(int argc, const char **argv) {
  ...
  std::vector<trace_command> commandlist = tracer.parse_commandlist_file(); // each mem cpy, and kernels
  ...
  for (unsigned i = 0; i < commandlist.size(); ++i) {
    ...
    if (commandlist[i].m_type == command_type::cpu_gpu_mem_copy) {
      ...
      kernel_trace_t kernel_trace_info = tracer.parse_kernel_info(commandlist[i].command_string); // This line reads kernel-{}.traceg file.
