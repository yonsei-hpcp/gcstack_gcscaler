...
boost::iostreams::filtering_istream *ifs_gzip;
...
trace_parser::trace_parser(const char *kernellist_filepath) {
  ...
  ifs_gzip = new boost::iostreams::filtering_istream;
}
...
trace_parser::parse_kernel_info(const std::string &kerneltraces_filepath) {
  is_gzip_file = false;
  const size_t last_dot_idx = std::string(kerneltraces_filepath.c_str()).rfind(".gz");
  if (std::string::npos != last_dot_idx) // .gz extension
    is_gzip_file = true;

  if (is_gzip_file) {
    ifs.open(kerneltraces_filepath.c_str(), std::ios_base::in | std::ios_base::binary); // e.g. /.../.../kernel-{}.traceg.gz - YA
    ifs_gzip->reset();
    ifs_gzip->push(boost::iostreams::gzip_decompressor());
    ifs_gzip->push(ifs);
  }
  else
    ifs.open(kerneltraces_filepath.c_str()); // e.g. /.../.../kernel-{}.traceg - YA
