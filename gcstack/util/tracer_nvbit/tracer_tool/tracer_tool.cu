...
uint64_t dynamic_kernel_limit_repeat_report = 0; // 0 means no repeat - JH
uint64_t dynamic_kernel_limit_repeat_noreport = 0; // 0 means no repeat - JH
...
void nvbit_at_init() {
  ...
  GET_VAR_INT(dynamic_kernel_limit_end, "DYNAMIC_KERNEL_LIMIT_ENDS", 0,
  ...
  GET_VAR_INT(
         ...
         "and cuProfilerStop. If set to 0, DYNAMIC_KERNEL_LIMIT, DYNAMIC_KERNEL_LIMIT_REPEAT options have no effect");
  GET_VAR_INT(dynamic_kernel_limit_repeat_report, "DYNAMIC_KERNEL_LIMIT_REPEAT_REPORT", 0,
              "repeat kerenl report : report this # and stop");
  GET_VAR_INT(dynamic_kernel_limit_repeat_noreport, "DYNAMIC_KERNEL_LIMIT_REPEAT_NOREPORT", 0,
              "repeat kerenl report : stop this # and report");
...
void nvbit_at_cuda_event(CUcontext ctx, int is_exit, nvbit_api_cuda_t cbid,
  ...
  if (cbid == API_CUDA_cuMemcpyHtoD_v2) {
    ...
    if (!is_exit) {
       ...
       * have been pushed */
      flush_channel<<<1, 1>>>();
      //printf("[nvbit_at_cuda_event] flush channel end\n");
      cudaDeviceSynchronize();
      ...
      //printf("[nvbit_at_cuda_event] pthread yield end\n");
      ...
      if (active_from_start && dynamic_kernel_limit_end && kernelid > dynamic_kernel_limit_end) {
        active_region = false;
        if (dynamic_kernel_limit_repeat_noreport) {
          dynamic_kernel_limit_start = kernelid + dynamic_kernel_limit_repeat_noreport;
          dynamic_kernel_limit_end = kernelid + dynamic_kernel_limit_repeat_noreport + dynamic_kernel_limit_repeat_report-1;
        }
      //printf("[nvbit_at_cuda_event] end\n");
      }
    }
