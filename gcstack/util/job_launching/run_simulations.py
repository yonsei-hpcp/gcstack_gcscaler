...
class ConfigurationSpec:
    ...
    def setup_run_directory(self, full_data_dir, this_run_dir, data_dir, appargs_subdir):
        ...
        if options.trace_dir != "":
            ...
            if "yeonan" in options.trace_dir: # for cactus
                app_subdir = appargs_subdir.replace("_", "")
                paths_to_try = [os.path.join(options.trace_dir, app_subdir)] + \
                glob.glob(os.path.join(options.trace_dir, "**", "**", app_subdir)) +\
                glob.glob(os.path.join(options.trace_dir, "**", app_subdir))
            else:
    ...
    def text_replace_torque_sim( self,full_run_dir,this_run_dir,benchmark, cuda_version, command_line_args,
        ...
        if options.trace_dir == "":
            ...
            txt_args = " -config ./gpgpusim.config -trace ./traces/kernelslist.g -launch_name " + options.launch_name # kernelslist 바꿔줭

        if os.getenv("TORQUE_QUEUE_NAME") == None:
            queue_name = "debug"    # partition name
        else:
            queue_name = os.getenv("TORQUE_QUEUE_NAME")

        # exclude_list = appropriate_exclude_list(mem_usage) # YA
