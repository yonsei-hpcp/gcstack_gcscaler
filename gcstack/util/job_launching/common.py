...
# def gen_all_apps_from_suite_list():
#     benchmarks = []
...
def print_stat(stat_name, all_named_kernels, apps_and_args, configs, stat_map, cfg_as_rows, do_averages, do_debug):
    csv_str = ""
    DIVISION = "-" * 100
    if cfg_as_rows:
            ...
            for appargs in apps_and_args:
                ...
                for kname in knames:
                    ...
                    if kname + appargs + config + stat_name in stat_map:
                        _stat = stat_map[kname + appargs + config + stat_name]
                        if (type(_stat) != list):
